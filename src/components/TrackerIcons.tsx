import { Territory } from "model/HistoryTracker";
...
export const Resource: React.FC<ResourceProps> = ({ amount, color = "black" }) => {
  if (amount === 0) return <></>;
  return (
    <svg
      id="resource"
...
export const Population: React.FC<PopulationProps> = ({ amount, color = "black" }) => {
  if (amount === 0) return <></>;
  return (
    <svg
      id="resource"
    ...
    >
      <g transform="translate(50,50)">
        <g transform="scale(0.6)">
          ...
          <polygon
            fill={territory.isMainCapital() ? territory.startingFaction().darkTone : territory.startingFaction().color}
            strokeWidth="1"
